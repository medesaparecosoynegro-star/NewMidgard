...
public class IntegerEditor implements StatEditor {
    ...
    public IntegerEditor(BiConsumer<MidgardItem, Integer> setter, String prompt) {
        this.setter = setter;
        this.prompt = prompt;
    }
    ...
    public void edit(Player player, ItemModule module, MidgardItem item, ItemEditionGui gui, ClickType clickType) {
        ...
        if (clickType.isRightClick()) {
            ...
            String msg = lang.getRawMessage("item.gui.editor.reset").replace("%s", prompt);
            player.sendMessage(MessageUtils.parse(msg));
        ...
        ChatInputListener.requestInput(player, (text) -> {
            try {
                ...
                player.sendMessage(lang.getMessage("item.gui.editor.invalid-number"));
                new ItemEditionGui(player, module, item, page).open();
            }
        });
    }
